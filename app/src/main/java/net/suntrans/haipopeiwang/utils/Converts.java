...
public abstract class Converts {
    ...
    *合并两张图片，返回合并后的Bitmap，如果前景图片不透明，则会完全遮盖
    ...
    public static String strToASCII(String string) {
        ...
        for (char aCh : ch) {
            sb.append(Integer.valueOf(aCh).intValue()).append("");
        }
        return sb.toString();
    }
    ...
    public static String strToASCIIHex(String string) {
        ...
        char[] ch = string.toCharArray();
        for (char aCh : ch) {
            int i1 = Integer.valueOf(aCh).intValue();
...
/*	public static String getLocalIpAddress() {
    ...
    public static String paddingHexString(String hexString, int targetLength) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < targetLength - hexString.length(); i++) {
            sb.append("0");
        }
        sb.append(hexString);
        return sb.toString();
